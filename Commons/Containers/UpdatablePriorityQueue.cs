...
namespace AdventOfCode.Commons.Containers
{
		...
		where TElement : notnull
	{
		...
		{
			this._nodes = Array.Empty<(TElement, int)>();
			this._elementIndex = new();
		}
		...
		public UpdateablePriorityQueue(int initialCapacity)
		{
			...
			this._nodes = new (TElement, int)[initialCapacity];
			this._elementIndex = new(initialCapacity);
		}
		...
		public UpdateablePriorityQueue(IEnumerable<(TElement Element, int Priority)> items)
		{
			...
			this._nodes = items.ToArray();
			this._elementIndex = new(this._nodes.Length);
		...
		{
			if (this._elementIndex.ContainsKey(element))
			{
				var index = this._elementIndex[element];
				if (priority >= this._nodes[index].Priority)
					return;

				this.MoveUpDefaultComparer((element, priority), index);
			}
			else
			{
		...
		public TElement Dequeue()
		{
			...
			this._elementIndex.Remove(element);
			this.RemoveRootNode();
			return element;
		}
		...
		public TElement EnqueueDequeue(TElement element, int priority)
		{
			if (this.Count != 0)
			{
				(TElement Element, int Priority) = this._nodes[0];
				this._elementIndex.Remove(Element);
		...
		public void EnqueueRange(IEnumerable<(TElement Element, int Priority)> items)
		{
			...
			if (collection is not null && (count = collection.Count) > this._nodes.Length - this.Count)
			{
			...
			if (this.Count == 0)
			{
				...
				{
					int i = 0;
					(TElement, int)[] nodes = this._nodes;
		...
		public void EnqueueRange(IEnumerable<TElement> elements, int priority)
		{
			...
			if (elements is ICollection<(TElement Element, int Priority)> collection &&
				(count = collection.Count) > this._nodes.Length - this.Count)
			{
				this.Grow(this.Count + count);
			}
		...
		public void TrimExcess()
		{
			...
			{
				Array.Resize(ref this._nodes, this.Count);
			}
		}
		...
		{
			int lastNodeIndex = --this.Count;

			if (lastNodeIndex > 0)
			{
				(TElement Element, int Priority) lastNode = this._nodes[lastNodeIndex];
		...
		private void Heapify()
		{
			for (int index = this.Count - 1; index >= 0; --index)
				this._elementIndex[this._nodes[index].Element] = index;
			...
			(TElement Element, int Priority)[] nodes = this._nodes;
			int lastParentWithChildren = GetParentIndex(this.Count - 1);
		...
		private void MoveUpDefaultComparer((TElement Element, int Priority) node, int nodeIndex)
		{
			...
			Debug.Assert(0 <= nodeIndex && nodeIndex < this.Count);
			...
			while (nodeIndex > 0)
			{
				...
				if (node.Priority < parent.Priority)
				{
					...
					this._elementIndex[parent.Element] = nodeIndex;
			...
			this._elementIndex[node.Element] = nodeIndex;
		}
		...
		private void MoveDownDefaultComparer((TElement Element, int Priority) node, int nodeIndex)
		{
			...
			while ((i = GetFirstChildIndex(nodeIndex)) < size)
			{
				...
				this._elementIndex[minChild.Element] = nodeIndex;
